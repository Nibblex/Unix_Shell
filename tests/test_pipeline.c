...
static scpipe pipe = NULL; /* para armar pipelines temporales */
...
{
	scpipe scmd = scpipe_new (SCOMMAND);
	scpipe_push_back (NULL, scmd);
	scpipe_destroy (scmd);
}
END_TEST
...
{
	pipe = scpipe_new (PIPELINE);
	scpipe_push_back (pipe, NULL);
	scpipe_destroy (pipe); pipe = NULL;
}
END_TEST
...
START_TEST (test_adding_emptying)
{
	...
	fail_unless (scpipe_is_empty (pipe), NULL);
	for (i=0; i<MAX_LENGTH; i++) {
		scpipe_push_back (pipe, scpipe_new (SCOMMAND));
		fail_unless (!scpipe_is_empty (pipe), NULL);
	}
	for (i=0; i<MAX_LENGTH; i++) {
		fail_unless (!scpipe_is_empty (pipe), NULL);
...
START_TEST (test_adding_emptying_length)
{
	...
	for (i=0; i<MAX_LENGTH; i++) {
		fail_unless (i == scpipe_length (pipe), NULL);
		scpipe_push_back (pipe, scpipe_new (SCOMMAND));
	}
	...
	fail_unless (0 == scpipe_length (pipe), NULL);
}
END_TEST
...
START_TEST (test_fifo)
{
	unsigned int i = 0;
	scpipe *scmds = NULL;
	scmds = calloc (MAX_LENGTH, sizeof(scpipe));
	for (i=0; i<MAX_LENGTH; i++) {
		scmds[i] = scpipe_new (SCOMMAND);
	}
	for (i=0; i<MAX_LENGTH; i++) {
		scpipe_push_back (pipe, scmds[i]);
	}
	for (i=0; i<MAX_LENGTH; i++) {
		fail_unless (scpipe_front (pipe) == scmds[i], NULL);
		scpipe_pop_front (pipe);
	}
}
END_TEST
...
START_TEST (test_front_idempotent)
{
	unsigned int i = 0;
	scpipe scmd = scpipe_new (SCOMMAND);
	scpipe_push_back (pipe, scmd);
	for (i=0; i<MAX_LENGTH; i++) {
		fail_unless (scpipe_front (pipe) == scmd, NULL);
	}
}
END_TEST
...
{
	scpipe scmd0 = scpipe_new (SCOMMAND);
	scpipe scmd1 = scpipe_new (SCOMMAND);
	scpipe_push_back (pipe, scmd0);
	scpipe_push_back (pipe, scmd1);
	fail_unless (scpipe_front (pipe) != scmd1, NULL);
}
END_TEST
...
/* Armamos un scpipe de n y contamos que haya n-1 '|'.
...
START_TEST (test_to_string)
{
	...
	for (i=0; i<MAX_LENGTH; i++) {
		scpipe_push_back (pipe, scpipe_new (SCOMMAND));
		scpipe_push_back (scpipe_front(pipe), bfromcstr ("gtk-fuse"));
	}
	pipeline_set_wait (pipe, false);
	str = pipeline_to_string (pipe);
...
void pipeline_memory_test (void) {
	...
	pipe = scpipe_new (PIPELINE);
	scpipe_push_back (pipe, scpipe_new (SCOMMAND));
	scpipe_push_back (scpipe_front (pipe), bfromcstr ("test"));
	scpipe_destroy (pipe);
}
